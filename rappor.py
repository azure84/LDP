...
# 각 cohort 마다
# 각 item의  bloom filter에서의 hash값을 기록해 놓는다. 이 값을 사용하여 나중에 변환된 hash 값을 아이템 값으로 돌려놓는다.
# (같은 아이템도 cohort 별로 다른 hash 값을 갖는다)
def making_dictionary(num_item,num_cohort,num_hash,num_bloom_bits):

    fw = open('dictionary.txt','w') #각 cohort마다 다르게 설정된 아이템별 hash값을 기록한다.기록 형식은 (아이템 이름, 변환된 해쉬값들)
...
def Rappor(user_data,num_bloom_bits,num_hash,p,q,f):
    ...
    blm_filter = bloom_filter(user_data,num_bloom_bits,num_hash)        # user의 item을 hash에 통과시켜 bloom_filter의 위치 지정
    str1 = ""                                                            
    for i in range(0,num_bloom_bits):
...
# RAPPOR 알고리즘에 사용되는 변수들에 대한 초기화 코드부분입니다.
    vector_c = {}
    vector_t = {}
    ...
    making_dictionary(num_item,num_cohort,num_hash,num_bloom_bits)                      # 각 cohort별 item의 hash 위치를 저장하는 making_dictionary 함수 호출
    user_data = making_input(num_reports, num_item, num_cohort, zipf_distribution)      # 각 user가 가진 item으로 bloom filter의 hash 위치를 찾는다.
